...
void main() {
  ...
  blocTest<LoadLevelsCubit, LoadLevelsState>(
      'should emits a [LoadLevelsLoading, LoadLevelssuccess with Levels list] when use case called and it return a list of Levels',
      setUp: () {
        when(() => loadLevelsUC.loadLevels())
            .thenAnswer((_) async => Right(levelsList));
      },
      build: () => LoadLevelsCubit(loadLevelsUC: loadLevelsUC),
      act: (cubit) => cubit.loadLevels(),
      expect: () => <LoadLevelsState>[
            LoadLevelsLoading(),
            LoadLevelsSuccess(levels: levelsList)
          ]);

  blocTest<LoadLevelsCubit, LoadLevelsState>(
      'should emits a [LoadLevelsLoading, LoadLevelsError with error msg] when use case return a failure',
      setUp: () {
        when(() => loadLevelsUC.loadLevels())
      ...
      expect: () => <LoadLevelsState>[
            LoadLevelsLoading(),
            LoadLevelsError(errorMessage: 'error from Server')
          ]);
}
